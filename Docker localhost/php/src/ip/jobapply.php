...
$post_jobs_id = isset($_GET['id']) ? $_GET['id'] : null;
...
$sql_check = "SELECT teacher_id FROM post_jobs WHERE id = ?";
...
$stmt_check->bind_param("i", $post_jobs_id);
$stmt_check->execute();
...
// ดึงข้อมูลนักศึกษา
$sql = "SELECT post_jobs.title, students.name, students.year, students.email, mojor.mojor_name 
        FROM students 
        JOIN mojor ON students.major_id = mojor.id 
        JOIN post_jobs ON post_jobs.id = ?
        WHERE students.id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $post_jobs_id, $students_id);
$stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["resume"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["resume"]["tmp_name"], $resume_path)) {
            ...
            $sql_insert = "INSERT INTO job_applications (student_id, post_jobs_id, GPA, Phone_number, resume) VALUES (?, ?, ?, ?, ?)";
            $stmt_insert = $conn->prepare($sql_insert);
            $stmt_insert->bind_param("iidss", $students_id, $post_jobs_id, $GPA, $phone, $resume_path);
            ...
            if ($stmt_insert->execute()) {
                ...
                $reference_table = 'job_appilcation'; // ตารางอ้างอิง
        ...
        <div class="form-card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" value="<?php echo htmlspecialchars($row['mojor_name']); ?>" readonly>
                </div>
